...
BACKUP_SCRIPT_PATH = "/opt/webkul_addons/wk_backup_restore/models/lib/saas_client_backup.py"
...
class Cronjob:
    ...
    @staticmethod
    def create_command(masterpswd, url, main_db, db_name, db_user, db_password, process_id, backup_location, storage_path):
        return "{} {} --mpswd {} --url {} --dbname {} --maindb {} --dbuser {} --dbpassword {} --processid {} --bkploc {} --path {} >> {} 2>&1".format(PYTHON_ENV, BACKUP_SCRIPT_PATH, masterpswd, url, db_name, main_db, db_user, db_password, process_id, backup_location, storage_path, LOG_FILE_PATH)
...
def add_cron(master_pass, url, main_db, db_name, db_user, db_password, process_id, backup_location, frequency, frequency_cycle, storage_path):
    _logger.info(locals())
    command = Cronjob.create_command(master_pass, url, main_db, db_name, db_user, db_password, process_id, backup_location, storage_path)
...
if __name__ == '__main__':
    ...
    # add_cron(master_pass, url, main_db, db_name, db_user, db_password, process_id, backup_location, frequency, frequency_cycle, storage_path)
